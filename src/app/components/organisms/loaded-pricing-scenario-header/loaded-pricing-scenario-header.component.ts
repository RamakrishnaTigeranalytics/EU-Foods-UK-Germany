  ...
  prevStep() {
    ...
    downloadWeekly(){
        ...
        this.pricingService.downloadPricingWeekly(this.pricingForm.value).subscribe(data=>{
            ...
            FileSaver.saveAs(
                blob,
                'pricingbulk_'+this.tenant + '_dowmload_' + new Date().getTime() + 'xlsx'
                )
                ,(err:any)=>{
            ...
            if(data){
                ...
                data.forEach(element => {
                    ...
                    this.lenta.push(this.formBuilder.group({
                        ...
                        inc_cogs: [0],
                        elasticity : [Number((element['elasticity']).toFixed(2))],
                        nsv_per_unit : [Number((element['nsv_unit_promo_base']).toFixed(2))],
                        inc_nsv_per_unit : [Number((element['nsv_unit_promo_inc']).toFixed(2))],
                        changed_nsv_per_unit : [Number((element['nsv_unit_promo_inc']).toFixed(2))],
                        ...
                        disable_internal : [true ? element['internal'] == 'not applicable' : false],
                        disable_private : [true ? element['private'] == 'not applicable' : false],
                        disable_others : [true ? element['others'] == 'not applicable' : false],
                        internal : [true ? element['internal'] == 'on' : false],
                        private : [true ? element['private'] == 'on' : false],
...
form['asp_count'] = form['asp_count'] + 1
    ...
    nsv_changed($event , i){
        ...
        let basensv = this.lenta.at(i).get('nsv_per_unit')?.value
        let nsv = this.lenta.at(i).get('inc_nsv_per_unit')?.value
        ...
        this.lenta.at(i).patchValue({
            "changed_nsv_per_unit" : Number((new_nsv).toFixed(2))
        })
    }
      rsp_changed($event , i){
          ...
          Object.values(form).forEach((e:any)=>{
            ...
            //   debugger
              this.lenta.push(this.formBuilder.group({
                ...
                nsv_per_unit :[Number(utils._divide(e.nsv_per_unit ,e.nsv_count )).toFixed(2)],
                //  [Number(e.nsv_per_unit.toFixed(2))],
                inc_nsv_per_unit : [0],
                changed_nsv_per_unit :[Number(utils._divide(e.nsv_per_unit ,e.nsv_count )).toFixed(2)],
                ...
                disable_promo : [false],    
                is_tpr_constant : [false],
                disable_internal : [e.internal],
                disable_private : [e.private],
...
let form={
        ...
        this.pricingArray.forEach(e=>{
            ...
            if(retailer in form){
                ...
                form[retailer]["nsv_per_unit"]  = form[retailer]["nsv_per_unit"]  +  e.nsv_per_unit,
            ...
            if(utils.isArray(this.chosen_promotion.meta)){
                (this.chosen_promotion.meta as MetaInfo[]).forEach(m=>{
                   ...
                   // console.log(ctrl , "contol chosen")
                    ...
                    let basensv = ctrl?.get('nsv_per_unit')?.value
                    let nsv = ctrl?.get('inc_nsv_per_unit')?.value
...
//         }
    ...
    ngOnInit() {
        ...
        if(location.pathname.includes('/uk/')){
